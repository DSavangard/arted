...
// Только при незалогиненном пользователе на странице my-account.

add_action('wp_head', function() {
    if (is_user_logged_in() || !is_account_page()) return;

    $lang = function_exists('arted_get_lang') ? arted_get_lang() : 'ru';
    $labels = ['ru' => 'Регистрация', 'en' => 'Register', 'fr' => "S'inscrire"];
    ...
    .arted-login-heading-row h2 { margin: 0 !important; }
    .arted-register-link {
        font-size: 11px;
        font-weight: 600;
    ...
    .arted-register-link:hover { background: #FD2E04; color: #070707 !important; }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!loginHeading) {
            document.querySelectorAll('.elementor-heading-title, h2, h3').forEach(function(el) {
                ...
                if (t === 'ВХОД' || t === 'LOGIN' || t === 'CONNEXION' || t === 'SE CONNECTER') loginHeading = el;
            });
        }
        ...
        if (!regForm) {
            document.querySelectorAll('h2, h3').forEach(function(el) {
                var t = el.textContent.trim().toUpperCase();
                if (t.includes('РЕГИСТРА') || t.includes('REGISTER') || t.includes('INSCRI')) regForm = el.parentElement;
            });
        }
        if (!regForm) return;
